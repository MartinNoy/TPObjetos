...
public class TarifaAlta extends Tarifa {
	...
	public void agregarDetalle(Double v1, Double v2, Double v3, Double v4) {
		/*
		 * DetalleAlta d = new DetalleAlta(detalleConcepto, unidad, supererLimite,
		 * valor); lstDetalle.add(d);
		 */
	...
	public float calcularTotalTarifa(int consumoPico, int consumoResto, int consumoValle) {
		...
		for (DetalleAlta d : lstDetalle) {
			if (auxilarConsumo < 300) {
				if (tensionContratada.charAt(0) == 'A' && tensionContratada.charAt(2) == '<') {
				...
				if (tensionContratada.charAt(0) == 'M' && tensionContratada.charAt(2) == '<') {
					if (d.getDetalleConcepto().contentEquals("Cargo Fijo")) {
						resultado = (float) (resultado + d.getValor());
					}
				...
				if (tensionContratada.charAt(0) == 'B' && tensionContratada.charAt(2) == '<') {
					if (d.getDetalleConcepto().contentEquals("Cargo Fijo")) {
						resultado = (float) (resultado + d.getValor());
					}
	...
	public List<DetalleAlta> pasarDetalle(int consumoPico, int consumoResto, int consumoValle) {
		List<DetalleAlta> detall = new ArrayList<DetalleAlta>();
		int auxilarConsumo = consumoPico + consumoResto + consumoValle;
		for (DetalleAlta d : lstDetalle) {
			if (auxilarConsumo < 300) {
				if (tensionContratada.charAt(0) == 'A' && tensionContratada.charAt(2) == '<') {
